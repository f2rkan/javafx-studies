...
public class TableviewExtendedController {
    ...
    void btn_goster_Click(ActionEvent event) {
    	...
    	if(kayitlar_table.getSelectionModel().getSelectedIndex() != -1){//integer bir deger döndürür -1 seçili ise bir satýr seçili deðil
    ...
    void btn_guncelle_Click(ActionEvent event) {
    	...
    	if(kayitlar_table.getSelectionModel().getSelectedIndex() != -1){//integer bir deger döndürür -1 seçili ise bir satýr seçili deðil
    		...
    		kayitlar_table.getItems().set(sira, kk); //istedigin sýraya istedigin guncellemeyi ata
    	}
    	else {
    		System.out.println("herhangi bir kayit secili degil");
    	}
    ...
    void btn_secili_Click(ActionEvent event) {
    	//CheckBox'ta seçilmiþ olan satýrlarýn deðerlerini alma
    ...
    void btn_yorum_Click(ActionEvent event) {
    	...
    	//yorum girilmiþ satýrlar belirlenerek yorum degerleri alýnsýn
    	for (Kayitlar_X islem : veriler) {
    ...
    void kayitlar_table_Click(MouseEvent event) {
    	//tablodan herhangi bir deger secildiginde calýsýr
    	...
    	//kayitlar icindeki getOrtalama degerini String'e dönüþtürüp at
    ...
    void initialize() {
        ...
        //neyin nerede tutulacagý:
        //1. id, tableColumn; 2. id, Kayitlar_X içerisindeki
    ...
    //üretilen butonlar icin olay atama
    @FXML
    void butonOlayi(ActionEvent event) {
    	//bir týklama olayý oldugu zaman hangisine týklandýgýný id'ye göre ayrýþtýrýrsýn
    	for (int i = 0; i < butonlar.length; i++) {
